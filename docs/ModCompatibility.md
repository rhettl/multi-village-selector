...
#### Requirement
Enable BCA village generation (default)

#### Config file
`config/cristellib/cobblemon_additions/structure_toggle_config.json5`
```json5
{
  "villages": {
    "bca:village": {
      ...
      "default_mid": true,
      "default_small": true,
      "witch_hut": true  // Optional: can leave disabled if you want
    }
  }
}
```
...
prevent_spawn: [],  // Do NOT add BCA here
...
#### Why
BCA replaces the vanilla structure set completely via datapack. MVS needs to intercept BCA villages directly since they're the only ones that will spawn.
...
| BCA | Override | ❌ No (keep enabled) | ✅ Yes | ❌ No |
