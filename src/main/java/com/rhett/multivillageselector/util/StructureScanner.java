    ...
    try {
        ...
        for (Path datapackPath : datapacksByName.values()) {
            try {
                ...
                MultiVillageSelector.LOGGER.warn("Failed to scan datapack {}: {}",
                    datapackPath.getFileName(), e.getMessage());
            }
        }

    } catch (Exception e) {
        MultiVillageSelector.LOGGER.error("Error scanning datapacks", e);
    }
        ...
        while (entries.hasMoreElements()) {
            ...
            if (entry.getName().contains("worldgen/structure_set") && entry.getName().endsWith(".json")) {
                try (InputStream is = zipFile.getInputStream(entry)) {
                    ...
                    MultiVillageSelector.LOGGER.debug("Failed to parse structure_set in datapack: {}", e.getMessage());
                }
            }
        }
        ...
        while (entries.hasMoreElements()) {
            ...
            if (name.contains("worldgen/structure/") &&
                ...
                try (InputStream is = zipFile.getInputStream(entry)) {
                    ...
                    MultiVillageSelector.LOGGER.debug("Failed to parse structure in datapack: {}", e.getMessage());
                }
            }
        }
    }
            ...
            structureSetFiles
                ...
                .forEach(path -> {
                    try {
                        ...
                        MultiVillageSelector.LOGGER.debug("Failed to parse structure_set in datapack directory: {}", e.getMessage());
                    }
                });
        }
    }
            ...
            structureFiles
                ...
                .forEach(path -> {
                    try {
                        ...
                        MultiVillageSelector.LOGGER.debug("Failed to parse structure in datapack directory: {}", e.getMessage());
                    }
                });
        }
    }
...
public static Map<Integer, Integer> calculateNormalizedWeights(
    ...
    MultiVillageSelector.LOGGER.debug("Weight normalization: spacing={}, rarityFactor={}, multiplier={}",
