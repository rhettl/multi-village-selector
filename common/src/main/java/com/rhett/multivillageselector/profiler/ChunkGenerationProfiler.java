...
public class ChunkGenerationProfiler {
    ...
    public static void start() {
        reset();
        running = true;
        MVSCommon.LOGGER.info("[MVS Profiler] Started - fly around to collect data");
    }
    ...
    public static void stop() {
        if (running) {
            running = false;
            logStats();
            MVSCommon.LOGGER.info("[MVS Profiler] Stopped");
        }
    }
    ...
    public static void logStats() {
        ...
        if (chunks == 0) {
            MVSCommon.LOGGER.info("[MVS Profiler] No chunks processed yet");
            return;
        }
        ...
        MVSCommon.LOGGER.info("========== MVS Performance Report ==========");
        MVSCommon.LOGGER.info("  Session: {} min | MVS enabled: {}", sessionDurationMs / 60000, MVSConfig.enabled);
        MVSCommon.LOGGER.info("  Chunks: {} ({}/sec)", chunks, String.format("%.1f", chunksPerSecond));
        MVSCommon.LOGGER.info("  Avg structure gen: {} µs/chunk", String.format("%.1f", avgTotalUs));
        MVSCommon.LOGGER.info("");
        MVSCommon.LOGGER.info("  --- MVS-Controlled (e.g., villages) ---");
        MVSCommon.LOGGER.info("  Spacing passed: {}", spacingPassed);
        MVSCommon.LOGGER.info("  Frequency failures: {} ({})", freqFailures, String.format("%.1f%%", freqFailRate));
        MVSCommon.LOGGER.info("  Selection attempts: {} | Successes: {} ({})",
            mvsSelections, mvsSuccesses, String.format("%.1f%%", mvsSuccessRate));
        MVSCommon.LOGGER.info("  Avg MVS selection time: {} µs  ← MVS overhead", String.format("%.1f", avgSelectionUs));
        MVSCommon.LOGGER.info("  Avg generation time: {} µs  ← Minecraft's work", String.format("%.1f", avgGenerationUs));
        MVSCommon.LOGGER.info("");
        MVSCommon.LOGGER.info("  --- Vanilla Structure Sets ---");
        MVSCommon.LOGGER.info("  Passthrough calls: {}", vanillaPasses);
        MVSCommon.LOGGER.info("  Avg time per call: {} µs", String.format("%.1f", avgVanillaPerCallUs));
        MVSCommon.LOGGER.info("");
        MVSCommon.LOGGER.info("  --- Comparison (MVS selection vs vanilla passthrough) ---");
        if (avgVanillaPerCallUs > 0) {
            ...
            MVSCommon.LOGGER.info("  MVS selection overhead: {} µs ({} vs vanilla)",
                String.format("%+.1f", overhead),
                String.format("%+.1f%%", overheadPercent));
        } else {
            MVSCommon.LOGGER.info("  MVS selection overhead: N/A (no vanilla calls)");
        }
        MVSCommon.LOGGER.info("=============================================");
    }
    ...
    public static void reset() {
        ...
        MVSCommon.LOGGER.info("[MVS Profiler] Counters reset");
    }
